    ...
    public async algorithm(req: Request, res: Response) {
        ...
        try {
            ...
            if (initial != null && final != null) {
                let actual = initial;
                let opened = [{ "station": actual.properties.id, "f": this.calc_distance(actual.geometry.coordinates, final.geometry.coordinates), "father": actual.properties.name }];
                let closed = [];
                while (opened.length > 0) {
                    ...
                    if (aux_mejor != undefined) {
                        ...
                        if (mejor != null) {
                            ...
                            closed.push(mejor);
                            g += this.calc_distance(initial.geometry.coordinates, mejor.geometry.coordinates);
                            actual = mejor;
                        }
                    }
