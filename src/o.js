class KNN {
	...
	calculateDistance(feature, prediction) {
		//console.log('im feature', feature);
		//console.log('im prediccion', prediction);
		...
		if (this.customDistanceFunction) {
			...
			throw new Error(
				'You should implement a custom distance calculator to this dimension: ' + 4
			);
		}
	}
		...
		for (let i = 0; i < this.dimension; i++) {
			...
			for (let normalize of toNormalize) {
				if (dimensionMinMax[0] == dimensionMinMax[1]) {
					...
					normalize[i] =
						(normalize[i] - dimensionMinMax[0]) /
						(dimensionMinMax[1] - dimensionMinMax[0]);
				}
			}
		}
		return toNormalize;
	...
	predict(features, prediction) {
		...
		for (let feature of features) {
			let distance = this.calculateDistance(
				feature.slice(0, this.dimension),
				prediction
			);
	...
	training(outputs, testSize, shuffleTimes, ktimes, normalize) {
		...
		const [testSet, trainingSet] = this.buildTrainingEnvironment(
			outputs,
			testSize,
			shuffleTimes,
			normalize
		...
		for (let k = 1; k <= ktimes; k++) {
			...
			let accuracy =
				(testSet.filter(test => {
					return (
						this.predict(trainingSet, test.slice(0, test.length - 1)) === test[3]
					);
				}).length /
					testSize) *
				100;
	...
	validateFeatureAndPredictionDimension(feature, prediction) {
		if (feature.length != this.dimension || prediction.length != this.dimension) {
			throw new Error(
				'The feature dimension or prediction dimensior are not compatible with the actual dimension: ' +
					this.dimension
			);
		}
		return;
